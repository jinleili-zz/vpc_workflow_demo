...
-- NOW() is STABLE rather than IMMUTABLE and therefore cannot appear in a
-- PostgreSQL partial-index predicate. Keep the lease column in the index so
-- both unlocked and expired rows can be selected efficiently.
CREATE INDEX IF NOT EXISTS idx_poll_tasks_next ON saga_poll_tasks(next_poll_at, locked_until);
