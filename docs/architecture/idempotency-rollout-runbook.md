...
The runner discovers every non-template database, skips databases without an NSP schema, applies migration `005` to Top and AZ service databases, and applies AZ-only migration `006` only where the `tasks` table exists. It uses `ON_ERROR_STOP`; both migrations are repeatable, and a failure stops the rollout before any application container should be upgraded.
